...
export function CardReviews({ userName, profilePicture, comment, rate, onClick }) {
    return (
        <>
            {/* Review Card */}
            <div className='bg-clr-black h-auto flex gap-5 rounded-2xl py-6 px-4' onClick={onClick}>
                {/* Profile Picture */}
                <img className='w-8 h-8 rounded-full border-2 border-white py-auto' src={profilePicture} alt="" />
                {/* Profile Picture */}

                {/* Review Content */}
                <div className="flex flex-col gap-1 my-auto">
                    <h5 className="text-white font-montserrat font-semibold">{userName}</h5>
