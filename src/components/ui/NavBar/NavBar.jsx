...
const NavBar = () => {
    ...
    if (!token) {
      return (
        <Link to="/Access_Panel/login">
          <SecondaryButton
            text="Únete"
            extraStyles={"px-4 py-2 w-full text-fs-med font-bold"}
          />
        </Link>
      );
    }
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 w-full flex items-center justify-between py-5 z-20 transition-all duration-300 shadow-lg bg-clr-white ${
          isScrolled ? "-translate-y-36" : "translate-y-0"
        }`}
      >
        <section className="w-full max-w-[100rem] flex items-center justify-between px-20 xs:px-6 md:px-10 m-auto">
          ...
          <div className="w-full sm:flex items-center justify-end gap-10 hidden">
